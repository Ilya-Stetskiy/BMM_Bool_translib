...
void run_single_function_checks(Report& rep, const std::vector<Dataset>& datasets) {
    ...
    for (const auto& ds : datasets) {
        for (Repr to : kAll) {
            ...
            if (!ds.exhaustive && to != Repr::Tt && to != Repr::Thr) {
                // ПРЕДОХРАНИТЕЛЬ: для больших (не exhaustive) реальных
                // датасетов допускаем только переводы в Tt/Thr — обе стороны
                // статически (дёшево, до какой-либо тяжёлой работы) проверяют
                // n <= kMaxTruthTableVars/аналог и отказывают быстро и
                // безопасно. Bdd/Anf НЕ имеют такой проверки и НЕ имеют
                // защиты от структурного взрыва: aig_to_bdd/anf_to_bdd могут
                // экспоненциально взорваться на "неудачном" графе
                // взаимодействия переменных (задокументировано и
                // подтверждено крахом процесса — Lace fatal error,
                // не ловится изнутри — в anf/README.md §5.2), а прогон ЭТОГО
                // теста на реальном EPFL-circuit (router, n=60) эмпирически
                // показал, что и aig_to_anf на настоящей (не синтетической)
                // схеме такого размера может не укладываться в разумное
                // время — вопреки более раннему выводу в aig/README.md
                // ("не найдено ни одного отказа"), который был получен
                // только на синтетических семействах (chain/star/block/mux/
                // random), не на реальных схемах. Гонять то и другое в
                // АВТОМАТИЧЕСКОМ, не присматриваемом прогоне (ctest) —
                // риск уронить/подвесить весь тестовый бинарник без
                // возможности поймать это изнутри процесса. Если нужно
                // всё же проверить конкретный большой случай — делайте это
                // отдельным запуском с внешним `timeout`, как рекомендует
                // anf/README.md §2.
                rep.line("- SKIP " + ds.name + ": " + repr_name(ds.repr) + "->" + repr_name(to) +
                         " (пропущено намеренно — предохранитель: для большого n без известной "
                         "структуры разрешены только Tt/Thr, у обоих есть дешёвая статическая "
                         "проверка размера)");
                continue;
            }
...
void run_round_trip_checks(Report& rep, const std::vector<Dataset>& datasets) {
    ...
    for (const auto& ds : datasets) {
        ...
        for (Repr via : kAll) {
            ...
            if (!ds.exhaustive && via != Repr::Tt && via != Repr::Thr) {
                // Тот же предохранитель, что и в run_single_function_checks —
                // см. комментарий там (для больших датасетов разрешены
                // только промежуточные Tt/Thr, у обоих дешёвая статическая
                // проверка размера; Bdd/Anf без такой защиты).

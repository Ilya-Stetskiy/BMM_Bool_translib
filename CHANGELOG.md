...
- CI (`.github/workflows/ci.yml`) — лёгкая версия (сборка зависимостей из
  ...
  cache miss).
- **Найдено первым живым прогоном CI (2026-08-11):** комментарий в
  `CMakeLists.txt` про `test_real_datasets` ("если данных нет — EPFL-случаи
  SKIP, не FAIL всего теста") не соответствует фактическому поведению
  `verify/real_datasets_tests.cpp` — без `benchmarks/data/epfl` тест реально
  проваливается (4 FAIL: не удалось загрузить `ctrl`/`int2float`/`cavlc`/
  `router.aig`). CI это обходит (скачивает EPFL-датасет, см. `ci.yml`), но
  сам разрыв между документацией и кодом теста — отдельный, не исправленный
  здесь баг (кто угодно, кто гоняет `ctest` локально без EPFL-данных, увидит
  тот же спонтанный FAIL).

...
- **CI (`.github/workflows/ci.yml`) подтверждён живым прогоном** на
  GitHub Actions (2026-08-11, PR #5) — оба job'а, `build-and-test` и
  `asan-ubsan`, зелёные (run 31470510014). Потребовалось три итерации,
  каждая нашла и починила реальную проблему, не гипотетическую:
...
- **`asan-ubsan` job подтверждён тем же зелёным прогоном** — `continue-on-
  error: true` снят (был временным, до первого живого прогона). Ни одного

...
//
// Поднято с 24 до 32 (ревью репозитория как библиотеки). kitty::
// dynamic_truth_table сама по себе не ограничивает 24 — внутреннее
// хранилище (std::vector<uint64_t>, побитовая упаковка) и вся его
// внутренняя сдвиговая арифметика безопасны минимум до n=37 (проверено по
// исходнику kitty/dynamic_truth_table.hpp: `_bits(num_vars <= 6 ? 1u :
// 1u << (num_vars - 6))` — ловушка была бы на num_vars=38, не раньше).
// 24 было историческим выбором самого bmm-translib ("разумный потолок"),
// не потолком снизу от библиотеки.
//
// Цена подъёма — ПАМЯТЬ, не вычислительная невозможность:
// - kitty::dynamic_truth_table (побитовая упаковка): 2 МБ при n=24 -> 512
//   МБ при n=32 (2^8 = 256x).
// - Функции, где Мёбиус-преобразование намеренно хранит 1 БАЙТ на строку
//   вместо бита ради кэш-локальности (anf_to_tt.cpp, tt_to_anf.cpp,
//   core/anf_repr.hpp::Anf::to_tt() — см. их комментарии): 16 МБ при n=24
//   -> 4 ГБ при n=32. Именно поэтому anf_to_tt/tt_to_anf теперь обязаны
//   ловить bad_alloc (см. CONVENTIONS.md п.2а) — на старом лимите такая
//   аллокация не могла реалистично провалиться, на новом может.
//
// Перед этим подъёмом найден и исправлен реальный баг, который поднятие
// лимита без аудита обнажило бы молча: bdd/bdd_to_anf.cpp использовал
// `1u << n` (32-битный литерал) в своём Мёбиус-фоллбэке — при n=32 это
// уже undefined behavior. Исправлено на 64-битные сдвиги (тот же паттерн,
// что и везде в проекте). Это был единственный такой случай в репозитории
// — остальной код уже везде использовал `1ULL <<`/`uint64_t{1} <<`.
inline constexpr uint32_t kMaxTruthTableVars = 32;

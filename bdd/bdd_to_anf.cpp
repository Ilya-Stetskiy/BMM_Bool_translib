...
class DenseBuilder {
public:
    static DenseBDD build(const sylvan::Bdd& bdd) {
        DenseBuilder builder;
        builder.traverse(bdd);
...
Result<Anf> bdd_to_anf(const Bdd& bdd) {
    ...
    try {
        ...
        detail::DenseBDD dense = detail::DenseBuilder::build(f_syl);
